...
export const handler = async (
  ...
  if (!ext) {
    return error(
      'VALIDATION_ERROR',
      `Unsupported contentType. Allowed: ${Object.keys(CONTENT_TYPE_EXT).join(', ')}`,
      400,
    );
  }
  ...
  const fileKey = `dogs/${dogId}/medical-records/${recordId}.${ext}`;
  const uploadUrl = await getPresignedPutUrl(bucket, fileKey, contentType, 300);

  return success({ uploadUrl, recordId, fileKey });
};
